...
int	sim_sleep(t_sim	*sim, long long duration)
{
	...
	while (sim->stop_reason == STOP_NONE)
	{
		...
		if (current_time >= end_time)
			break;
		pthread_cond_timedwait(&sim->monitor_cond, &sim->arb_mutex, &ts);
	}
...
int	sim_request_stop(t_sim *sim, t_stop_reason reason, int coder_id, long long terminal_time)
{
	if (sim == NULL)
		return (0);
	if (reason == STOP_NONE)
